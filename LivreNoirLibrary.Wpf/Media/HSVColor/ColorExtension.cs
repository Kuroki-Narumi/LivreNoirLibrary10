...
namespace LivreNoirLibrary.Media
{
    public static partial class ColorExtension
    {
        ...
        {
            if (TryParseColorCode(colorCode, out var a, out var r, out var g, out var b))
            {
                return Color.FromArgb(GetByte(a), GetByte(r), GetByte(g), GetByte(b));
            }
        ...
        public static bool TryParseToColor(this string colorCode, out Color color)
        {
            if (TryParseColorCode(colorCode, out var a, out var r, out var g, out var b))
            {
                color = Color.FromArgb(GetByte(a), GetByte(r), GetByte(g), GetByte(b));
                return true;
            }
            color = default;
            return false;
