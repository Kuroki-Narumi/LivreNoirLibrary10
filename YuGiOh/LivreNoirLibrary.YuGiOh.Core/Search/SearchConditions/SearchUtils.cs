...
namespace LivreNoirLibrary.YuGiOh.Search
{
    public static class SearchUtils
    {
        ...
        public static bool IsMatch<T>(IEnumerable<T> set, SortedSet<T> compareTo, MatchType type)
            => compareTo.Count is 0 || type switch
            {
                MatchType.All => compareTo.IsSubsetOf(set),
                MatchType.Minimum => compareTo.IsSupersetOf(set),
                MatchType.Perfect => compareTo.SetEquals(set),
                _ => compareTo.Overlaps(set)
            };
