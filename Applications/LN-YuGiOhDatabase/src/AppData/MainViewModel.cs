...
namespace LivreNoir.YuGiOhDatabase
{
    public class MainViewModel : AppSettingsBase
    {
        ...
        public const string Filename = "Main";
        ...
        public double WindowLeft { get; set => SetValue(ref field, value); }
        public double WindowTop { get; set => SetValue(ref field, value); }
        [JsonIgnore]
        public Version Version { get; set => SetValue(ref field, value); } = UpdateInfo.GetCurrentVersion();
        ...
        {
            base.OnLoad();
            Regulation.Instance.LoadFile(GetFilePath(AppName, nameof(Regulation)));
            if (Json.TryOpen<Card[]>(GetFilePath(AppName, nameof(OriginalCards)), out var ary1))
            {
                OriginalCards.AddRange(ary1);
            }
            Deck.LoadFile(GetFilePath(AppName, nameof(Deck)));
        ...
        {
            base.OnSave();
            Regulation.Instance.SaveJson(GetFilePath(AppName, nameof(Regulation)));
