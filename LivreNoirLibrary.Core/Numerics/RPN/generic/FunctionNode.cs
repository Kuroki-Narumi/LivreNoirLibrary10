...
namespace LivreNoirLibrary.Numerics
{
    ...
    {
        public delegate T Func(ReadOnlySpan<T> operands, IDictionary<string, T> variables);
        ...
        protected virtual FunctionNode GetFunctionNode(string symbol, int operandCount)
        {
            if (_node_map.TryGetValue((symbol, operandCount), out var node))
            {
                return node;
            }
            return new($"{symbol}{{{operandCount}}}", operandCount, operandCount is 1 ? (s, v) => s[0] : (s, v) => T.Zero);
        }

        protected static FunctionNode CreateValueNode(T value) => new(value.ToString()!, 0, (s, v) => value);
        protected static FunctionNode CreateVariableNode(string symbol) => new(symbol, 0, (s, v) => v[symbol]);
    }
}
