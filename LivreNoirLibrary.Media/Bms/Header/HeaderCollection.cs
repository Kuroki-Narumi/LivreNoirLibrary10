...
namespace LivreNoirLibrary.Media.Bms
{
    public class HeaderCollection : IJsonWriter, IDumpable<HeaderCollection>
    {
        ...
        private readonly HashSet<HeaderType> _decimal_headers = [
        ...
        private readonly SortedDictionary<HeaderType, decimal> _decimalValues = [];
        private readonly SortedDictionary<HeaderType, string> _stringValues = [];
        private readonly List<(string Key, string Value)> _sub = [];

        public PlayerType Player { get => (PlayerType)GetInt(HeaderType.Player, (int)Constants.DefaultPlayer); set => SetInternal(HeaderType.Player, (decimal)value); }
        ...
        public decimal Bpm { get => GetDecimal(HeaderType.Bpm, Constants.DefaultBpm); set => SetInternal(HeaderType.Bpm, value); }
        ...
        public Rank Rank { get => (Rank)GetInt(HeaderType.Rank, (int)Constants.DefaultRank); set => SetInternal(HeaderType.Rank, (decimal)value); }
        public double Total { get => (double)GetDecimal(HeaderType.Total, Constants.DefaultTotal); set => SetInternal(HeaderType.Total, (decimal)value); }
        ...
        public LongNoteMode LnMode { get => (LongNoteMode)GetInt(HeaderType.LnMode, (int)Constants.DefaultLnMode); set => SetInternal(HeaderType.LnMode, (decimal)value); }
        public double ExRank { get => (double)GetDecimal(HeaderType.DefExRank, Constants.DefaultExRank); set => SetInternal(HeaderType.DefExRank, (decimal)value); }
        ...
        public bool IsEmpty() => _decimalValues.Count is 0 && _stringValues.Count is 0 && _sub.Count is 0;
        ...
        {
            if (inheritance is not Inheritance.Parent && _decimalValues.TryGetValue(type, out var value))
            {
                return (int)value;
            }
        ...
        public decimal GetDecimal(HeaderType type, decimal ifNone = 0, Inheritance inheritance = Inheritance.Inherited)
        {
            if (inheritance is not Inheritance.Parent && _decimalValues.TryGetValue(type, out var value))
            {
                return value;
            ...
            if (inheritance is not Inheritance.Actual && Parent is not null)
            {
                return Parent.GetDecimal(type, ifNone, inheritance);
            }
            return ifNone;
        }
        ...
        private void SetInternal(HeaderType type, decimal value)
        {
            if (value is < 0 || (GetDecimal(type, 0, Inheritance.Parent) == value))
            {
                _decimalValues.Remove(type);
            }
            else
            {
                _decimalValues[type] = value;
            }
        }
        ...
        internal void SetAuto(string key, string value)
        {
            ...
            {
                if (_decimal_headers.Contains(type))
                {
                    if (decimal.TryParse(value, out var v))
                    {
                        _decimalValues[type] = v;
                    }
                }
                else
                {
        ...
        internal void Dump(RawData.BmsTextWriter writer, int radix)
        {
            for (var t = HeaderType.Player; t is <= HeaderType.Comment; t++)
            {
                ...
                if (_decimal_headers.Contains(t))
                {
                    if (_decimalValues.TryGetValue(t, out var value) && GetDecimal(t, 0, Inheritance.Parent) != value)
                    {
        ...
        {
            writer.Write(_decimalValues.Count);
            foreach (var (key, value) in _decimalValues)
            {
        ...
        {
            foreach (var (k, v) in src._decimalValues)
            {
                _decimalValues[k] = v;
            }
