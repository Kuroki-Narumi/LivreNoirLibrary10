...
namespace LivreNoirLibrary.Media.Bms
{
    ...
    {
        public bool ApplyMultiDef(MultiDefOptions options, string directory, out int newMaxIndex) => ApplyMultiDef(options, directory, out newMaxIndex, null, CancellationToken.None);
        public bool ApplyMultiDef(MultiDefOptions options, string directory, out int newMaxIndex, ProgressReporter? reporter, CancellationToken c)
        {
            var modified = false;
            ...
            if (modified)
            {
                ...
                foreach (var (originalIndex, duplicateCount, defValue, replaceList) in CollectionsMarshal.AsSpan(replace))
                {
                    ...
                    foreach (var (note, replaceIndex) in CollectionsMarshal.AsSpan(replaceList))
                    {
                        ...
                        if (replaceIndex > newMaxIndex)
                        {
                            newMaxIndex = replaceIndex;
                        }
                    }
                ...
                if (options.InsertDefIndex)
                {
                    ...
                    for (var i = 0; i < maxDefIndex; i++)
                    {
                        ...
                        if (insert.TryGetValue(i, out var inserted))
                        {
                            ...
                            newMaxIndex = Math.Max(newMaxIndex, i + offset);
                        }
                    }
