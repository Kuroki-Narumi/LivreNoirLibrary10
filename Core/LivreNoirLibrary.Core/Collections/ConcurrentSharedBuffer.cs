...
{
    public class ConcurrentSharedBuffer<T> : DisposableBase, IList<T>
    {
        ...
        public ConcurrentSharedBuffer(int capacity = DefaultCapacity)
        {
        ...
        public static implicit operator Span<T>(ConcurrentSharedBuffer<T> buffer) => buffer.AsSpan();
        public static implicit operator ReadOnlySpan<T>(ConcurrentSharedBuffer<T> buffer) => buffer.AsSpan();
        ...
        {
            private readonly ConcurrentSharedBuffer<T> _source;
            private readonly int _size;
            private int _index;

            internal Enumerator(ConcurrentSharedBuffer<T> source)
            {
                _source = source;
                _size = source._size;
                _index = -1;
