...
namespace LivreNoirLibrary.IO
{
    ...
    {
        public static void Dump<T>(BinaryWriter writer, T obj) where T : IDumpable => obj.Dump(writer);

        public static void Dump(BinaryWriter writer, byte[] buffer) => WriteWithSize(writer, buffer);
