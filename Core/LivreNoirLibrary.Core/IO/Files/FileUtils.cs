...
namespace LivreNoirLibrary.IO
{
    public static class FileUtils
    {
        ...
        public static bool TryGetMediaFileName(string? path, [MaybeNullWhen(false)] out string actualPath) => Exts.TryGetCompatible(path, out actualPath, Exts.VisualExts);

        public static IEnumerable<string> GetAllAudioFileNames(string? path) => Exts.GetAllCompatible(path, Exts.AudioExts);
        public static IEnumerable<string> GetAllMediaFileNames(string? path) => Exts.GetAllCompatible(path, Exts.VisualExts);
    }
}
