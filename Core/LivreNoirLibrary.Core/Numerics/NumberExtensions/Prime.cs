...
namespace LivreNoirLibrary.Numerics
{
    public static partial class NumberExtensions
    {
        ...
        private static readonly Dictionary<long, long> _division_buffer = [];
        ...
        public static ReadOnlySpan<PrimeFactor> Division(this long val)
        {
            if (!_division_cache.TryGetValue(val, out var ary))
            {
                ...
                for (var p = 3L; p <= limit; p += 2)
                {
                    var (q, r) = long.DivRem(v, p);
                    while (r is 0)
                    {
                        AddFactor(p);
                        (q, r) = long.DivRem(q, p);
                ...
                _division_cache.Add(val, [.. dic.Select(kv => new PrimeFactor(kv))]);
            }
            return ary;
        }
