...
namespace LivreNoirLibrary.Numerics
{
    public static unsafe partial class FFT
    {
        ...
        public static ReadOnlySpan<double> GetHamming(int ww) => _hamming.GetOrAdd(ww, _create_hamming);
        public static ReadOnlySpan<float> GetHamming32(int ww) => _hamming_32.GetOrAdd(ww, _create_hamming_32);
        public static ReadOnlySpan<double> GetHammingComplex(int ww) => _hamming_c.GetOrAdd(ww, _create_hamming_c);
        public static ReadOnlySpan<float> GetHammingComplex32(int ww) => _hamming_c_32.GetOrAdd(ww, _create_hamming_c_32);

        public static void ApplyHamming(Span<double> buffer, int ww = DefaultWindowWidth)
        {
            var hamming = GetHamming(ww);
            buffer.Multiply(hamming);
        }

        public static void ApplyHammingComplex(Span<double> buffer, int ww = DefaultWindowWidth)
        {
            var hamming = GetHammingComplex(ww);
            buffer.Multiply(hamming);
        }

        public static void ApplyHamming(Span<float> buffer, int ww = DefaultWindowWidth)
        {
            var hamming = GetHamming32(ww);
            buffer.Multiply(hamming);
        }

        public static void ApplyHammingComplex(Span<float> buffer, int ww = DefaultWindowWidth)
        {
