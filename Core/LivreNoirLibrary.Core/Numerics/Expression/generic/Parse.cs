...
namespace LivreNoirLibrary.Numerics
{
        ...
        where T : unmanaged, INumber<T>
    {
        ...
        static readonly ThreadLocal<Stack<Token>> _tokenStack = new(() => new());
        static readonly ThreadLocal<Stack<int>> _opCountStack = new(() => new());

        public bool TryParse(ReadOnlySpan<char> expression, [MaybeNullWhen(true)]out Exception exception, ICharTypeProvider? charTypeProvider = null)
        {
            charTypeProvider ??= CharTypeProvider.Default;
            Clear();
            var list = _nodes;
            var stack = _tokenStack.Value!;
            var functionOperandCounts = _opCountStack.Value!;
            try
            {
                ...
                foreach (var (i, type, symbol) in new SymbolEnumerator(expression, charTypeProvider))
                {
                    switch (type)
                    {
