    ...
    public static partial class Exts
    {
        ...
        public static bool TryGetCompatible(string? path, [MaybeNullWhen(false)]out string actualPath, params ReadOnlySpan<string> exts)
        {
            if (!string.IsNullOrEmpty(path))
            {
                ...
                {
                    var path2 = Path.ChangeExtension(path, ext);
        ...
        public static IEnumerable<string> GetAllCompatible(string? path, IEnumerable<string> exts, bool checkExist = true)
        {
            if (!string.IsNullOrEmpty(path))
            {
                ...
                {
                    var path2 = Path.ChangeExtension(path, ext);
                    if (path != path2 && (!checkExist || File.Exists(path2)))
                    {
                        yield return path2;
                    }
                }
