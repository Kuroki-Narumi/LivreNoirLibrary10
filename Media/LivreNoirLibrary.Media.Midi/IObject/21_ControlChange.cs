...
{
    public sealed class ControlChange(CCType type, int value, int? ext = null) : IObject, IObject<ControlChange>, IEquatable<ControlChange>
    {
        public const int ExtDisabled = Events.ControlChange.ExtDisabled;
        ...
        public void ExtendToEvent(RawTimeline timeline, int channel, long tick, Rational pos, long ticksPerWholeNote)
        {
            ...
            switch (type)
            {
                case CCType.PolyphonicKeyPressure:
                    timeline.Add(tick, new Events.PolyphonicKeyPressure(channel, value, ext));
                    break;
                case CCType.ProgramChange:
                    if (ext is not ExtDisabled)
                    {
                        timeline.Add(tick, new Events.ControlChange(channel, CCType.BankSelect, (ext >> 8) & 0x7F));
                        timeline.Add(tick, new Events.ControlChange(channel, CCType.BankSelect_LSB, ext & 0x7F));
                    }
                    timeline.Add(tick, new Events.ProgramChange(channel, value));
                    break;
                case CCType.ChannelPressure:
                    timeline.Add(tick, new Events.ChannelPressure(channel, value));
                    break;
                case CCType.PitchBend:
                    timeline.Add(tick, new Events.PitchBend(channel, value));
                    break;
                case CCType.SequenceNumber:
                    timeline.Add(tick, new Events.SequenceNumberEvent(value));
                    break;
                case CCType.ChannelPrefix:
                    timeline.Add(tick, new Events.ChannelPrefix(value));
                    break;
                case CCType.Port:
                    timeline.Add(tick, new Events.Port(value));
                    break;
                case CCType.RPN:
                    timeline.Add(tick, new Events.ControlChange(channel, CCType.RPN_MSB, (ext >> 8) & 0x7F));
                    timeline.Add(tick, new Events.ControlChange(channel, CCType.RPN_LSB, ext & 0x7F));
                    if (ext is not 0x7F7F)
                    {
                        if (Events.ControlChange.NeedsDataMSB(ext))
                        {
                            timeline.Add(tick, new Events.ControlChange(channel, CCType.DataEntry, (value >> 8) & 0x7F));
                            timeline.Add(tick, new Events.ControlChange(channel, CCType.DataEntry_LSB, value & 0x7F));
                        }
                        else
                        {
                            timeline.Add(tick, new Events.ControlChange(channel, CCType.DataEntry, value));
                        }
                    }
                    break;
                case CCType.NRPN:
                    timeline.Add(tick, new Events.ControlChange(channel, CCType.NRPN_MSB, (ext >> 8) & 0x7F));
                    timeline.Add(tick, new Events.ControlChange(channel, CCType.NRPN_LSB, ext & 0x7F));
                    if (ext is not 0x7F7F)
                    {
                        timeline.Add(tick, new Events.ControlChange(channel, CCType.DataEntry, value));
                    }
                ...
                case CCType.KeySwitch:
                    ...
                    break;
                default:
                    timeline.Add(tick, new Events.ControlChange(channel, type, value, ext));
                    break;
            }
        }
