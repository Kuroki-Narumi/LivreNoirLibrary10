...
namespace LivreNoirLibrary.Media.Bms
{
    public partial class BmsTextReader
    {
        ...
        [GeneratedRegex(@"^\s*#BASE\s+(\d+)", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.RightToLeft)]
        private static partial Regex Regex_Radix { get; }
        [GeneratedRegex(@"^\s*#LNOBJ\s+(\w+)", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.RightToLeft)]
        ...
        public BmsTextReader(Stream stream)
        {
            ...
            RawText = text;
            var match = Regex_Radix.Match(text);
            Radix = match.Success ? int.Parse(match.Groups[1].Value) : Constants.Base_Default;
            match = Regex_LnObj.Match(text);
            LnObj = match.Success ? BasedNumber.ParseToLong(match.Groups[1].Value, Radix) : 0;
        }
            ...
            foreach (var line in RawText.EnumerateLines())
            {
                try
                {
                    ...
                    if (span.Length is > 0)
                    {
                        ...
                        {
                            parser.AddComment(line.ToString());
                        }
                    }
                    lineNumber++;
                }
