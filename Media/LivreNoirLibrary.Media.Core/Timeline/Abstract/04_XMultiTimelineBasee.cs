...
namespace LivreNoirLibrary.Media
{
        ...
        where TOperator : IPositionOperator<TX>, new()
    {
        ...
        public int RemoveAll(Predicate<TX, TValue> predicate) => RemoveAllCore(predicate, GetPositionIndex());
        public int RemoveAll(Predicate<TX, TValue> predicate, Range<TX> range) => RemoveAllCore(predicate, GetPositionIndex(range));

        protected int RemoveAllCore(Predicate<TX, TValue> predicate, (int Start, int Length) range)
        {
            var l = range.Length;
            if (l is <= 0) { return 0; }
        ...
        public void MoveAll(Predicate<TX, TValue> predicate, Func<TX, TX> converter) => MoveAllCore(predicate, converter, GetPositionIndex());
        public void MoveAll(Predicate<TX, TValue> predicate, Func<TX, TX> converter, Range<TX> range) => MoveAllCore(predicate, converter, GetPositionIndex(range));

        protected void MoveAllCore(Predicate<TX, TValue> predicate, Func<TX, TX> converter, (int Start, int Length) range)
        {
            var l = range.Length;
            if (l is <= 0) { return; }
