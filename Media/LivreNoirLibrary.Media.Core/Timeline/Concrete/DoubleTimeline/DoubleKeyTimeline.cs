...
namespace LivreNoirLibrary.Media
{
    public partial class DoubleKeyTimeline<TKey, TValue> : XYSingleTimelineBase<TKey, double, TValue, Operator_double> where TKey : struct
    {
        ...
        public void CopyTo(IXYSingleTimeline<TKey, double, TValue> destination, Range<decimal> srcRange) 
            => CopyToCore(destination, _value_list.Keys, p => p.IndexRange(srcRange), 0);
        public void CopyTo(IXYSingleTimeline<TKey, double, TValue> destination, Range<decimal> srcRange, double destOffset) 
