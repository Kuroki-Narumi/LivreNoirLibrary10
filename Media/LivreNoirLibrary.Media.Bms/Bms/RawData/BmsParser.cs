...
namespace LivreNoirLibrary.Media.Bms
{
    public partial class BmsParser : IBmsParser
    {
        ...
        private readonly Dictionary<DefType, Dictionary<long, double>> _conductor_defs = [];
        ...
        {
            _current.Comments.Add(_comments.ToString());
            _comments.Clear();
        }
        ...
        void IBmsParser.AddComment(string value) => _comments.AppendLine(value);
        ...
        void IBmsParser.AddConductorDef(DefType type, long key, double value) => _conductor_defs.GetOrAdd(type)[key] = value;
        ...
        void IBmsParser.AddBar(int number, Channel channel, ReadOnlySpan<char> value)
        {
            if (channel is Channel.Bar)
            {
                ...
                {
                    _current.Data.Bars.Set(number, Rational.ConvertBySBT(v, DenominatorLimit));
                }
            }
            else if (channel.IsConductor())
            {
                _current.AddUnProcessedLine(number, channel, value.ToString());
            }
            else
            {
            ...
            foreach (var (channel, list) in state.UnProcessedLines)
            {
                ...
                foreach (var (number, line) in CollectionsMarshal.AsSpan(list))
                {
                    ...
                    for (var i = 0; i < den; i++)
                    {
                        ...
                        {
                            tl.Add(new(number, i, den), new ConductorNote(channel, Rational.ConvertBySBT(value, limit)));
                        }
                        span = span[2..];
                    }
                }
