...
{
    public record BmsonData : IFile<BmsonData>
    {
        public static BmsonData Open(string path) => General.Open(path, Json.Load<BmsonData>);

        public void Save(string path) => Save(path, false, false);
        public void Save(string path, bool pretty = true, bool ext = true) => General.Save(path, s => Dump(s, pretty), ext ? ExtRegs.Bmson : null, Exts.Bmson);
