...
{
    public sealed class AudioFileReader : WaveStream, IWaveProvider, ISampleProvider, IMediaDecodeContext
    {
        private readonly IAudioDecodeContext _decoder;
        ...
        private AudioFileReader(IAudioDecodeContext decoder)
        {
            _decoder = decoder;
        ...
        public static IAudioDecodeContext DecoderAutoOpen(string path)
        {
            var stream = File.OpenRead(path);
            return ExtRegs.Wav.IsMatch(path) && WaveInfo.IsSupported(stream) ? new WaveDecoder(stream, false) : new AudioDecoder(stream, false);
        }
        ...
        {
            IAudioDecodeContext decoder = WaveInfo.IsSupported(stream) ? new WaveDecoder(stream, leaveOpen) : new AudioDecoder(stream, leaveOpen);
            return new(decoder);
        }

        void IMediaDecodeContext.Seek(Rational position) => _decoder.Seek(position);
