...
namespace LivreNoirLibrary.Media.Ogg
{
    ...
    {
        public static unsafe uint Update(uint crc, ReadOnlySpan<byte> data)
        {
            var i = 0;
            ...
            fixed (byte* p = data)
            {
                for (; size is >= 8; size -= 8, i += 8)
                {
                    crc ^= (uint)((p[i] << 24) | (p[i + 1] << 16) | (p[i + 2] << 8) | (p[i + 3]));
                    crc = _lookup[7][crc >> 24] ^ _lookup[6][(crc >> 16) & 0xFF] ^
                          _lookup[5][(crc >> 8) & 0xFF] ^ _lookup[4][crc & 0xFF] ^
                          _lookup[3][p[i + 4]] ^ _lookup[2][p[i + 5]] ^
                          _lookup[1][p[i + 6]] ^ _lookup[0][p[i + 7]];
                }
                for (; size is > 0; size--, i++)
                {
                    crc = (crc << 8) ^ _lookup[0][((crc >> 24) & 0xFF) ^ p[i]];
                }
            }
