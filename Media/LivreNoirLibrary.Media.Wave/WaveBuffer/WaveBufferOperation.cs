...
namespace LivreNoirLibrary.Media.Wave
{
    ...
    public partial class WaveBuffer
    {
        ...
        public const double PitchFactor = 1200;
        public const double InvertPitchFactor = 1 / PitchFactor;

        public static double Pitch2Ratio(double pitch) => Math.Pow(2, pitch * InvertPitchFactor);

        public static double Ratio2Pitch(double ratio)
        {
            if (ratio is <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(ratio), $"value must be > 0. (given:{ratio})");
            }
            return Math.Log2(ratio) * PitchFactor;
        }
