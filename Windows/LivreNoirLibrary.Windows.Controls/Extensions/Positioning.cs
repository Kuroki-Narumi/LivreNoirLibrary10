...
namespace LivreNoirLibrary.Windows.Controls
{
    public static partial class ControlExtension
    {
        ...
        {
            Rect bounds;
            if (listView.TryGetFirstDescendant<VirtualizingStackPanel>(out var stack))
            {
            ...
            if (index is >= 0)
            {
                if (listView.TryGetFirstDescendant<ListViewItem>(d => i++ == index, out var item))
                {
        ...
        {
            Rect bounds;
            if (listView.TryGetFirstDescendant<ScrollContentPresenter>(out var content))
            {
            ...
            if (predicate is not null && listView.TryGetFirstDescendant(predicate, out var item))
            {
                var pos = item.PointToScreen(new(0, item.ActualHeight));
                return new Point(Math.Clamp(pos.X, bounds.X, bounds.Right), Math.Clamp(pos.Y, bounds.Y, bounds.Bottom));
            }
        ...
        public static Point GetLeftTopPosition(this Control control, TreeView treeView, Predicate<TreeViewItem>? predicate)
        {
            Rect bounds;
            Point pos;
            if (treeView.TryGetFirstDescendant<ScrollContentPresenter>(out var content))
            {
            ...
            if (predicate is not null && treeView.TryGetFirstDescendant(predicate, out var item))
            {
