...
namespace LivreNoirLibrary.Windows.Controls
{
    public partial class RadarChart : TextViewerBase
    {
        ...
        private static (double, double) GetSinCos(double angle)
        {
            if (!_angle_cache.TryGetValue(angle, out var value))
            {
                var th = Math.PI * 2.0 * angle;
                var (s, c) = Math.SinCos(th);
                value = (-s, c);
                _angle_cache.Add(angle, value);
            }
            return value;
        }

        private List<Point> _point_cache = [];
        ...
        protected override void OnRender(DrawingContext dc)
        {
            ...
            // 目盛り分割数
            var step = Step;
            var stepInvert = 1d / step;
            // 項目名オフセット
            var fontSize = _fontSize = FontSize;
            var caption_ox = HorizontalCaptionOffset.Validate(fontSize);
            var caption_oy = VerticalCaptionOffset.Validate(fontSize * 0.5);
            for (var i = 0; i < count; i++)
            {
                // 基本位置
                var (sin, cos) = GetSinCos(angle + (isCcw ? 1.0 : -1.0) * i * countInvert);
                var x = radius * cos;
                var y = radius * sin;
                // 分割線座標のリスト
                var list = _point_cache;
                list.Clear();
                for (var j = 1; j < step; j++)
                {
                    list.Add(new(ox + x * j * stepInvert, oy + y * j * stepInvert));
                }
                _points.Add(new()
                {
                    X = x,
            ...
            // レーダー最大値と最小値指定の処理
            var (minD, maxD) = _values.MinMax();
            var min = (float)MinValue.Validate(minD);
            var max = (float)MaxValue.Validate(maxD);
            // グラデーション基準点
            var mid = (float)MidValue.Validate((maxD + minD) * 0.5);
            ...
            _value_rect = new Rect(minX, minY, maxX - minX, maxY - minY).ToInt32Rect();
            ...
            for (var i = 1; i <= count; i++)
            {
                ...
                if (pen_step is not null)
                {
                    ...
                    for (int j = 0; j < len; j++)
                    {
                        dc.DrawLine(pen_step, get_p(pre.Steps[j]), get_p(now.Steps[j]));
                    }
                }
                if (pen_frame is not null)
                {
                    var nowP = get_p(now.FramePoint);
                    dc.DrawLine(pen_frame, get_p(center), nowP);
                    dc.DrawLine(pen_frame, get_p(pre.FramePoint), nowP);
                }
            }
            /* Draw Fill */
            ...
            for (var i = 1; i <= count; i++)
            {
                var now = _points[i];
                var pre = _points[i - 1];
                var p0 = get_p(new(ox + pre.ValueX, oy + pre.ValueY));
                var p1 = get_p(new(ox + now.ValueX, oy + now.ValueY));
            ...
            void DrawCaption_Text(DrawingContext dc, double cx, double cy, string? text, int lineOffset)
            {
                if (round)
                {
                    cx = Math.Round(cx);
            ...
            void DrawCaption_Image(DrawingContext dc, double cx, double cy, ImageSource source, bool valueVisible)
            {
                var x = cx - source.Width * 0.5;
                var y = cy - source.Height * 0.5;
                if (valueVisible)
                {
                    y -= fontSize * 0.5;
                }
