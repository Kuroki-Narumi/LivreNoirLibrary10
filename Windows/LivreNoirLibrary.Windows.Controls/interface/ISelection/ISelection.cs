...
namespace LivreNoirLibrary.Windows.Controls
{
    ...
    public static class ISelectionExtension
    {
            ...
            where T : ISelection
        {
            ...
            {
                selection.Mode = ElementSelectionMode.New;
            }
            else if (KeyInput.IsAltDown())
            {
                if (KeyInput.IsShiftDown())
                {
                    selection.Mode = ElementSelectionMode.Intersect;
                }
                else
                {
                    selection.Mode = ElementSelectionMode.Except;
                }
            }
            else if (KeyInput.IsShiftDown())
            {
                selection.Mode = ElementSelectionMode.Union;
            }
            else
            {
                selection.Mode = ElementSelectionMode.New;
            ...
            void MouseUp(object sender, MouseButtonEventArgs e)
            {
                ...
                selection.Mode = ElementSelectionMode.None;
                if (hide)
                {
                    selection.Hide();
                }
