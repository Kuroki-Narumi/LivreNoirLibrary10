...
namespace LivreNoirLibrary.Windows.Controls.Bms
{
    public class NoteRectBase : SelectableObject
    {
        ...
        protected void DrawText(DrawingContext ctx, int y, string? text) => ctx.DrawNoteText(X + 2, y, text);
        protected void DrawText(DrawingContext ctx, int y, string? text, Color color) => ctx.DrawNoteText(X + 2, y, text, color);
    }
}
