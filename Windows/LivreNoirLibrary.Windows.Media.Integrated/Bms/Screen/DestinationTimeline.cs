...
{
    public class DestinationTimeline : DoubleTimeline<double>
    {
        ...
        public double GetBlended(double time, double slope)
        {
            var poss = _pos_list;
            var values = _value_list;
            var index = poss.BinarySearch(time);
            if (index is >= 0)
            {
                return values[index];
            }
            else
            {
                ...
                if (slope is > 0 && rightIndex < values.Count)
                {
                    ...
                    var dif = (time - leftPos) / (poss[rightIndex] - leftPos);
                    if (slope is not 1)
                    {
