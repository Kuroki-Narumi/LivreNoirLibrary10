...
namespace LivreNoirLibrary.Windows.Media
{
    public static partial class Bitmap
    {
        ...
        public static unsafe void FillRect(this WriteableBitmap bitmap, int x, int y, int width, int height, Color color)
        {
            ...
            var c = ColorOperation.ToUInt(color);
            bitmap.Lock();
            try
            {
                var offset = GetUIntPtr(bitmap, x, y);
            ...
            try
            {
                foreach (var (left1, right1, y) in Enum_FillTriangle(x0, y0, x1, y1, x2, y2))
                {
                    if ((uint)y < (uint)srcHeight)
                    {
                        ...
                        var ptr = GetUIntPtr(bitmap, left2, y);
                        new Span<uint>(ptr + left2, width).Fill(c);
                    }
                }
            }
            finally
        ...
        private static (int Min, int Max) MinMax(int v1, int v2, int v3) => (Math.Min(Math.Min(v1, v2), v3), Math.Max(Math.Max(v1, v2), v3));
        ...
        internal readonly struct ColorGetter
        {
            ...
            public Color Get(int num)
            {
                ...
                {
                    color = HsvColor.GetBlended(Color1, Color2, num * InvertDen).ToColor();
        ...
        public static unsafe void DrawBorder2(this WriteableBitmap bitmap, int thickness, Color color)
        {
            using var ptr = new BitmapPointer(bitmap);
            var width = bitmap.PixelWidth;
            var height = bitmap.PixelHeight;
            var stride = bitmap.BackBufferStride;
            var buffer = ArrayPool<byte>.Shared.Rent(ptr.BufferLength);
            var intColor = ColorOperation.ToUInt(color) & ~ColorOperation.Mask_Alpha;
            buffer.CopyFrom(ptr);
            try
            {
                // パス1: 水平方向のA最大値をチェック
                ...
                {
                    var sourceSpan = ptr.AsSpan(y);
                    var bufferSpan = buffer.AsSpan(y * stride, stride);
                    for (var dx = 1; dx <= thickness; dx++)
                    {
                        var xx = dx * 4;
                        bufferSpan.Max(sourceSpan[xx..stride]);
                        bufferSpan[xx..stride].Max(sourceSpan);
                    }
                }
                ptr.AsSpan().CopyFrom(buffer);
                // パス2: 垂直方向のA最大値をチェック
                for (var y = 0; y < height; y++)
                {
                    var bufferSpan = ptr.AsSpan(y);
                    for (var dy = Math.Max(-y, -thickness); dy < 0; dy++)
                    {
                        bufferSpan.Max(buffer.AsSpan((y + dy) * stride, stride));
                    }
                ...
                ptr.AsUIntSpan().And(ColorOperation.Mask_Alpha);
                ptr.AsUIntSpan().Or(intColor);
            }
            finally
            {
        ...
        public static unsafe void DrawBorder(this WriteableBitmap bitmap, int thickness, Color color, bool keepSource = false)
        {
            ...
            var width = bitmap.PixelWidth;
            var height = bitmap.PixelHeight;
            var buffer = ArrayPool<uint>.Shared.Rent(width * height);
            var intColor = ColorOperation.ToUInt(color) & ~ColorOperation.Mask_Alpha;
            try
            {
                fixed (uint* bufferPtr = buffer)
                {
                    ...
                    ExConsole.Write($"Setup: {System.Diagnostics.Stopwatch.GetElapsedTime(t0):mm\\:ss\\.ffff}");
                    t0 = System.Diagnostics.Stopwatch.GetTimestamp();
                    DetectAlphaCore((uint*)ptr.Pointer, bufferPtr, dxCache, width, height, thickness, intColor);
                    ExConsole.Write($"Detect Alpha: {System.Diagnostics.Stopwatch.GetElapsedTime(t0):mm\\:ss\\.ffff}");
                    t0 = System.Diagnostics.Stopwatch.GetTimestamp();

                    if (keepSource)
                    {
                        var srcPtr = (byte*)bufferPtr;
                        var dstPtr = ptr.Pointer;
                        var max = width * height;
                        for (var i = 0; i < max; i++, srcPtr += 4, dstPtr += 4)
                        {
                            var dstAlpha = ColorUtils.GetFloat(dstPtr[(int)ColorIndex.A]);
                            if (dstAlpha is 0)
                            {
                                *(uint*)dstPtr = *(uint*)srcPtr;
                            }
                            else if (dstAlpha is not 1)
                            {
                                // アルファブレンド
                                var srcAlpha = ColorUtils.GetFloat(srcPtr[(int)ColorIndex.A]);
                                var srcBlend = srcAlpha * (1f - dstAlpha);
                                var alpha = dstAlpha + srcBlend;
                                for (var j = 0; j < 3; j++)
                                {
                                    var c = (dstAlpha * dstPtr[j] + srcBlend * srcPtr[j]) / alpha;
                                    dstPtr[j] = (byte)(c + 0.5f);
                                }
                                dstPtr[(int)ColorIndex.A] = ColorUtils.GetByte(alpha);
                            }
                        }
                        ExConsole.Write($"Alpha Blend: {System.Diagnostics.Stopwatch.GetElapsedTime(t0):mm\\:ss\\.ffff}");
                    }
                    else
                    {
                        ptr.AsUIntSpan().CopyFrom(buffer);
                        ExConsole.Write($"Copy: {System.Diagnostics.Stopwatch.GetElapsedTime(t0):mm\\:ss\\.ffff}");
                    }
                }
            }
            finally
        ...
        /// </summary>
        private static unsafe void DetectAlphaCore(uint* source, uint* bufferPtr, int* dxCache, int width, int height, int thickness, uint intColor)
        {
            // 行ごとに並列処理
            Parallel.For(0, height, y =>
            {
                ...
                for (var x = 0; x < width; x++)
                {
                    ...
                    bufferPtr[index] = intColor | (alpha & ColorOperation.Mask_Alpha);
                    index++;
                }
            });
        }
